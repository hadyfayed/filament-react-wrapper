...
export class FilamentReactAdapter {
  ...
  private static scanAndRenderComponents(): void {
    const containers = document.querySelectorAll('[data-react-component]:not([data-react-rendered])');
    
    // Process containers in batches to avoid blocking the main thread
    const processContainers = (containers: NodeListOf<Element>, startIndex: number, batchSize: number) => {
  ...
  private static renderComponent(element: HTMLElement): void {
    ...
    if (!element.id) {
      element.id = `react-${componentName}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    }
        ...
        try {
            ...
            universalReactRenderer.render({
              ...
              onDataChange: (data) => {
                ...
                if (statePath) {
                  element.dispatchEvent(new CustomEvent('react-data-change', {
              ...
              onError: (error) => {
                ...
                // Emit error event
                element.dispatchEvent(new CustomEvent('react-error', {
                  detail: { error: error instanceof Error ? error.message : String(error), componentName },
                  bubbles: true,
                }));
              }
            });
